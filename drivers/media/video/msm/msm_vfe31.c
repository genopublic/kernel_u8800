...
#else
    /*when in recoring mode, go this "if" branch to remove the preview frame when previous frame is preview frame*/
	if(recording_flag)
	    {

			  if (out_bool && recording_frame_flag) {
				ping_pong = msm_io_r(vfe31_ctrl->vfebase +
					VFE_BUS_PING_PONG_STATUS);
				...
				/*process the preview frame, set recording_frame_flag for PREVIOUS_FRAME_IS_PREVIEW*/
                  recording_frame_flag = PREVIOUS_FRAME_IS_PREVIEW;
			} else {
				vfe31_ctrl->outpath.out0.frame_drop_cnt++;
				CDBG("path_irq_0 - no free buffer!\n");
			}
