...
static void tpa2028d1_amplifier_on(void)
{
    ...
    if(ret)
        TPA_DEBUG_TPA("failed to turn on tpa2028d1_r_amplifier\n");
    else
        TPA_DEBUG_TPA("tpa2028d1_r_amplifier_on\n");
    /*
    for(i = 1; i<8; i++)
    {
...
static void tpa2028d1_amplifier_4music_on(void)
{
    ...
    if(ret)
        ...
        TPA_DEBUG_TPA("tpa2028d1_r_amplifier_4music_on\n");
...
static int tpa2028d1_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	g_client = client;
	gpio_tlmm_config(GPIO_CFG(125, 0, GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA),GPIO_ENABLE);
    gpio_set_value(125, 1);/* enable spkr poweramp */
    msleep(10);
